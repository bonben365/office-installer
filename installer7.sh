...
############################################## Start functions
   ...
   if ($2013Pro.Checked -eq $true) {$productId = 'ProfessionalRetail';Invoke-Command $install}
   if ($2013Std.Checked -eq $true) {$productId = 'StandardRetail';Invoke-Command $install}
   if ($2013ProjectPro.Checked -eq $true) {$productId = 'VisioProRetail';Invoke-Command $install}
   if ($2013VisioPro.Checked -eq $true) {$productId = 'ProjectProRetail';Invoke-Command $install}
...
############################################## Start group boxes
   ...
   $groupBoxUninstall.Location = New-Object System.Drawing.Size(150,130) 
